...
サブピクセル強調は、ステレオアルゴリズムの解像度を上げる簡単な方法を提供する。 
新しい視差画像を得るために、近傍コストを通して2次曲線をフィッティングする：

$$
D_{SE}(\mathbf{p}) = d - \frac{C_+ - C_-}{2(C_+ - 2C + C_-)}, \tag{14}
$$

ここで $d = D_{INT}(\mathbf{p}), C_-=C_{SGM}(\mathbf{p}, d-1), C = C_{SGM}(\mathbf{p}, d), C_+=C_{SGM}(\mathbf{p}, d+1)$